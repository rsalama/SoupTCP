 ...
 */
package stirling.nasdaqomx.souptcp30

import java.nio.ByteBuffer

import stirling.nasdaqomx.souptcp30._
 ...
 * 2.1
 */
case class Debug(text: String) extends Packet

object Debug extends PacketType {
  val minLength = 2

  def parse(buffer: ByteBuffer) = Debug(text = ByteBuffers.getAlpha(buffer))

  def format(buffer: ByteBuffer, text: String) = {
    buffer.put('+'.toByte)
 ...
 * 2.2.1
 */
case class LoginAccepted(session: String, sequenceNumber: Long) extends Packet
...
object LoginAccepted extends PacketType {
  ...
  def parse(buffer: ByteBuffer) = LoginAccepted(
    ...
    sequenceNumber = ByteBuffers.getAlpha(buffer, 20).trim.toLong)

  def format(buffer: ByteBuffer, session: String, sequenceNumber: Long) = {
    buffer.put('A'.toByte)
    buffer.put(alpha(session, 10))
    buffer.put(numeric(sequenceNumber, 20))
    buffer.put(LF)
  }
}
 ...
 * 2.2.2
 */
case class LoginRejected(rejectReasonCode: Byte) extends Packet

object LoginRejected extends PacketType {
  val length = 3

  def parse(buffer: ByteBuffer) = LoginRejected(rejectReasonCode = buffer.get())

  def format(buffer: ByteBuffer, rejectReasonCode: Byte) = {
    buffer.put('J'.toByte)
 ...
 * 2.2.3
 */
case class SequencedData(message: Array[Byte]) extends Packet

object SequencedData extends PacketType {
  val minLength = 2

  def parse(buffer: ByteBuffer) = SequencedData(message = ByteBuffers.unwrap(buffer))
 ...
 * 2.2.5
 */
case object EndofSession extends Packet with PacketType {
  val length = 2

  def parse(buffer: ByteBuffer) = EndofSession
...
case class LoginRequest(
  ...
  requestedSequenceNumber: Long) extends Packet
...
object LoginRequest extends PacketType {
  ...
  def parse(buffer: ByteBuffer) = LoginRequest(
    ...
    requestedSequenceNumber = ByteBuffers.getAlpha(buffer, 20).trim.toLong)
  ...
  def format(
    ...
    buffer.put(numeric(requestedSequenceNumber, 20))
    buffer.put(LF)
  }
}
...
case class UnsequencedData(
  message: Array[Byte]) extends Packet
